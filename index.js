    ...
    try {
        ...
        db.serialize(async () => {
            ...
            for (const comment of comments) {
                ...
                let commentText_authContent_data;
                if(commentText === config.commentAuth_en){
                    commentText_authContent_check = 'en';
                    commentText_authContent_data = `Hello, ${authorName},\n`;
                }else{
                    if(commentText === config.commentAuth_ja){
                        commentText_authContent_check = 'ja';
                        commentText_authContent_data = `${authorName} さん、こんにちは。\n`;
                    };
                };

                // 対象のコメントの場合
                if (!!commentText_authContent_check) {
                            ...
                            if( checkCount < 1 ){
                                ...
                                if (user_data) {
                                    ...
                                    if (public_metadata && public_metadata.tag && public_metadata.tag === 'authentic') {
                                        ...
                                        user.addComment({
                                            user: authorName,
                                            content: `${commentText_authContent_data}${config[`commentAuth_replyNotification_warn_${commentText_authContent_check}`]}${config[`commentAuth_replyNotification_automatically_${commentText_authContent_check}`]}`,
                                        }, (err) => {
                                        ...
                                        user.addComment({
                                            user: authorName,
                                            content: `${commentText_authContent_data}${config[`commentAuth_replyNotification_successful_${commentText_authContent_check}`]}${config[`commentAuth_replyNotification_automatically_${commentText_authContent_check}`]}`,
                                        }, (err) => {
                                            if (err) return console.error("コメントの追加エラー:", err);
                                        });
                                        // データを追加
                                    ...
                                    user.addComment({
                                        user: authorName,
                                        content: `${commentText_authContent_data}${config[`commentAuth_replyNotification_error_${commentText_authContent_check}`]}${config[`commentAuth_replyNotification_automatically_${commentText_authContent_check}`]}`,
                                    }, (err) => {
                                        if (err) return console.error("コメントの追加エラー:", err);
                                    });
                                    // データを追加
